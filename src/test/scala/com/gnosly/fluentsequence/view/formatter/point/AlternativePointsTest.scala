...
class AlternativePointsTest extends FunSuite with Matchers {
  ...
  test("resolve") {
    AlternativePoints(ALTERNATIVE_ID, Variable2DPoint(Fixed1DPoint(0), Fixed1DPoint(0)))
      .toPoints(null) shouldBe Alternative.topLeft(ALTERNATIVE_ID) -> Fixed2dPoint(0, 0) :: Nil
  }
}
