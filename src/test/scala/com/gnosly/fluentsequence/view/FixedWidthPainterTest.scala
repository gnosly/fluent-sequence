...
import com.gnosly.fluentsequence.view.model.component.{ActivityComponent, ActorComponent}
...
class FixedWidthPainterTest extends FlatSpec with Matchers {
	...
	it should "render user box" in {
		...
		painter.paint(user, Map(
			...
			Coordinates.Activity.topLeft(0, 0) -> new Fixed2dPoint(3, 4),
			Coordinates.Activity.bottomLeft(0, 0) -> new Fixed2dPoint(3, 8),
			Coordinates.Activity.rightPointStart(0, 0, 0) -> new Fixed2dPoint(6, 5)
		), canvas)
	...
	it should "render user box with a bisignal exiting" in {
		...
		painter.paint(user, Map(
			...
			Coordinates.Actor.bottomMiddle(0) -> new Fixed2dPoint(4, 4),
			Coordinates.Activity.topLeft(0, 0) -> new Fixed2dPoint(3, 4),
			Coordinates.Activity.bottomLeft(0, 0) -> new Fixed2dPoint(3, 7),
			Coordinates.Activity.rightPointStart(0, 0, 1) -> new Fixed2dPoint(6, 5),
			Coordinates.Activity.leftPointStart(1, 0, 1) -> new Fixed2dPoint(20, 5)
		), canvas)
	...
	it should "render user box with many activities" in {
		...
		painter.paint(user, Map(
			...
			Coordinates.Activity.topLeft(0, 0) -> new Fixed2dPoint(3, 4),
			Coordinates.Activity.bottomLeft(0, 0) -> new Fixed2dPoint(3, 7),
			Coordinates.Activity.topLeft(0, 1) -> new Fixed2dPoint(3, 9),
			Coordinates.Activity.bottomLeft(0, 1) -> new Fixed2dPoint(3, 12)
		), canvas)
