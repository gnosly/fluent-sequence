...
import com.gnosly.fluentsequence.view.fixedwidth.painter.{Box, FixedWidthPainter}
...
class FixedWidthBiSignalFormatterTest extends FunSuite with Matchers {
	...
	test("first bisignal on the right") {
		...
		val point = new FixedWidthBiSignalFormatter(new FixedWidthPainter()).formatOnRight(signal)
