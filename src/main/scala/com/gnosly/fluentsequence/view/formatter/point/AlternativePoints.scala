...
case class AlternativePoints(alternativeId: Int, topLeft: Point2d) extends Pointable {
  override def toPoints(pointMap: PointMap): Seq[(String, Fixed2dPoint)] =
    Coordinates.Alternative.topLeft(alternativeId) -> topLeft.resolve(pointMap) :: Nil
}
