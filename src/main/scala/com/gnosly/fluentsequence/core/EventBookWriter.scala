...
class EventBookWriter {
	...
	private def encode(e: EventBookEvent): String = e.event match {
		...
		case s:CALLED => s"CALLED|${s.who.actorType.name()}|${s.who.name}|${s.something}|${s.toSomebody.actorType.name()}|${s.toSomebody.name}"
		case s:REPLIED => s"REPLIED|${s.who.actorType.name()}|${s.who.name}|${s.something}|${s.toSomebody.actorType.name()}|${s.toSomebody.name}"
