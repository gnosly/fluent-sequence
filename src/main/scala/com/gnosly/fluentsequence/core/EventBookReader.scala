...
class EventBookReader {
	...
	def decode(line: String): Event = {
		...
		eventType match {
			...
			case "CALLED" => CALLED(Actor(typeFrom(columns(1)), columns(2)),columns(3),Actor(typeFrom(columns(4)), columns(5)))
			case "REPLIED" => REPLIED(Actor(typeFrom(columns(1)), columns(2)),columns(3),Actor(typeFrom(columns(4)), columns(5)))
